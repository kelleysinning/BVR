...
# 1/7/2025 for SEA (niche overlap) and flow 
...
discharge_data <- read.csv("SI_discharge.csv") # Qualifier A means it was approved, P = provisional

# SI overlap from 5/24/21-10/7/2024
# ALL available SI data so far
OVERLAP <- read.csv("OVERLAP.csv")

# Convert dates to Date class
discharge_data$Date <- as.Date(discharge_data$Date, format = "%m/%d/%Y")
OVERLAP$Sampling_date <- as.Date(OVERLAP$Sampling_date, format = "%m/%d/%Y")
discharge_data$Date <- as.Date(discharge_data$Date)
OVERLAP$Sampling_date <- as.Date(OVERLAP$Sampling_date)
...
SlidWin <- slidingwin(xvar = list(climate = discharge_data$Discharge_cfs),
                      ...
                      bdate = OVERLAP$Sampling_date, #bdate = date variable for biological dataset
...
# p > 0.05 original slidingwin result is LIKELY to be an issue of overfitting
