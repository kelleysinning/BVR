...
discharge_data <- read.csv("discharge_data_2026.csv")

# Diatom conc. from benthotorch from 1/15/2023-8/27/2025
# ALL available diatom data so far
diatoms_discharge <- read.csv("didymo_over_time.csv")

# Convert dates to Date class because it can be finnicky otherwise
discharge_data$Date <- as.Date(discharge_data$Date, format = "%Y-%m-%d")
diatoms_discharge$Sampling_date <- as.Date(diatoms_discharge$Sampling_date, format = "%Y-%m-%d")
...
SlidWin[[1]]$BestModel # BestModel: is a model object showing the relationship between climate and biological date within the strongest climate window.

# When climate = 0, the predicted value of diatoms is 1.6779964
# For each 1-unit increase in climate (e.g., 1 cfs of discharge), the model predicts a decrease of -0.0005184 units in diatom concentration.
...
# Blue shows little relationships
...
plothist(dataset = Output, datasetrand = OutputRand)
# As we can see below, the value of ΔAICc from the real data (dashed line) is very different to those values obtained from randomized data, so we can feel confident our results from SlidWin did not occur by chance
```
...
SlidWin_single <- singlewin(xvar = list(climate = discharge_data$Discharge_cfs),
                            ...
                            range = c(29, 20),
                            type = "relative", 
                            stat = c("max"), # choose max bc this was the best model output
                            func = "lin")
